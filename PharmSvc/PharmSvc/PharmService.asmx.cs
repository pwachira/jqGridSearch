...
namespace Pharmacy
{
    ...
    public class PharmSvc : System.Web.Services.WebService
    {
        ...
        public ArrayList GetSites()
        {
            ...
            string sysEnv = ConfigurationManager.AppSettings["SysEnvironment"];
            ArrayList arraySites = new ArrayList();
            using (SiteClassesDataContext sdc = new SiteClassesDataContext(ConfigurationManager.ConnectionStrings[sysEnv + "ClientDictionaryConnectionString"].ToString()))
            {
        ...
        //[WebMethod]
        //[ScriptMethod(ResponseFormat = ResponseFormat.Json/*, UseHttpGet = true*/)]
        //public jqGridGeneric<SiteDrugRoute> PharamcyDrugsProfiles(int[] pharmIds)
        //{
        //    string ConnString;
        //    //string db = "DMSS";
        //    //ConnString = Connections.CreateConnectionString(SiteCode, Environment, db);
        ...
        //            //var sdr = QueryRepo.QryableSiteDrugRoutes();
        //            //filters f = js.Deserialize<filters>(filters);
        //            //var o = Search.FilterEntity<SiteDrugRoute>(f, sdr).OrderBy<SiteDrugRoute>(sidx, sord);
        //            //return new jqGridGeneric<SiteDrugRoute>(page, rows, o);
        //        }
        ...
        public jqGridGeneric<PharmacyDrug> PharmacyDrugs(string SiteCode, string Environment, int rows, int page,  string filters, string sidx, string sord)
        {
            ...
            }))
            {
                ...
                {
                    var pdwc = QueryRepo.PharmDrugsWithClass(cdc);
                    filters f = js.Deserialize<filters>(filters);
                    var o = Search.FilterEntity<PharmacyDrugWithClass>(f, pdwc);
                    var pd = QueryRepo.PharmacyDrugs(o,sidx,sord);
                    return new jqGridGeneric<PharmacyDrug>(page, rows, pd) ;
                }
            }
        }
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json,UseHttpGet=true)]
        public List<Profile> ProfilesMapValues()
        {
            ...
            }))
            {
                using (CssPharmDataContext cdc = new CssPharmDataContext())
                {
                    return cdc.CSS_GetTreatmentProfiles().Select(p => new Profile{ 
                    TreatmentProfileName = p.TreatmentProfileName,
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public jqGridGeneric<SiteDrug> NewSiteDrugs(string SiteCode, string Environment, DateTime Start, DateTime End,string filters, string sidx, string sord, int page, int rows)
        {
            string ConnString;
            string db = "DMSS";
            ConnString = Connections.CreateConnectionString(SiteCode, Environment, db);
            ...
            }))
            {
                using (DmssClassesDataContext ddc = new DmssClassesDataContext(ConnString))
                {
                    var orders = QueryRepo.NewDmssOrders(ddc, Start, End);
                    filters f = js.Deserialize<filters>(filters);
                    var o = Search.FilterEntity<SiteDrug>(f, orders).OrderBy<SiteDrug>(sidx, sord);
                    return new jqGridGeneric<SiteDrug>(page, rows, o);                    
