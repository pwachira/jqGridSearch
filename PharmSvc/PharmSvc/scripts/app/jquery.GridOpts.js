/// <reference path ="../lib/jquery-1.7.2.min.js"/>
/// <reference path ="jquery.ColModels.js"/>
/// <reference path ="../lib/jquery-ui-1.8.19.custom.min.js"/>
        ...
        var pharmMthds = {
            ...
            setDefaultGridOptions: function () {
                $.extend($.jgrid.defaults, {
                    ...
                    data: [{}, {}, {}],
                    cmTemplate: { searchoptions: { sopt: ['cn', 'eq', 'ne']},searchrules:{required:true} },
            ...
            siteRoutesGridOpts: function (options) {
                var rtColModel = colModels().routesColModel();
                return {
            ...
            siteDrugRoutesGridOpts: function (options) {
                var drColModel = colModels().drugRoutesColModel();
                return {
                    url: 'PharmService.asmx/SiteDrugRoutes',
                    colNames: ['Id', 'SiteRouteId', 'SiteRouteCode', 'SiteDrugName', 'PharmacyDrugId','MappedPharmacyDrug','TreatmentProfile'],
                    colModel: drColModel,
                    sortorder: 'ASC',
                    sortname: 'Id',
            ...
            facilitySplitGridOpts: function (options) {
                var fColModel = colModels().facilitySplitColModel();
                return {
            ...
            archMinMaxGridOpts: function (options) {
                var minMaxColModel = colModels().minMaxColModel();
            ...
            mapProfilesGridOpts: function (options) {
                var tpColModel = colModels().mapProfilesColModel();
                return {
            ...
            siteDrugsGridOpts: function (options) {
                var sdColModel = colModels().siteDrugsColModel();
                return {
            ...
            archiveSubgridOpts: function (row_id, subgridpager, form) {
                var opts = colModels().componentsColModel(),
            ...
            rxSubgridOpts: function (row_id, subgridpager) {
                var opts = colModels().rxSubGridColModel(), form = $('#siteForm').serializeForm(), pdata = $.extend(form, { RxId: parseInt(row_id, 10) });
                return {
            ...
            componentsGridOpts: function (options) {
                ...
                return {
                    url: 'PharmService.asmx/GetComponents',
                    colNames: ['HeaderRowTk', 'MessageControlId', 'OrderRowTk', 'RxcNum', 'Type', 'RxcCodeId',
                'RxcCodeText', 'CodeAlt', 'CodeCoding', 'Amount', 'Units', 'Strength', 'StrengthUnits', 'FacilityId', 'SendingApplication'],
                    colModel: opts,
            ...
            distinctRxcGridOpts: function (options) {
                var opts = colModels().distinctRxcColModel();
                return {
                    url: 'PharmService.asmx/DistinctRxc',
                    colNames: ['RxcNum', 'RxeCodeText', 'RxeCodeId', 'Type',
                    'RxcCodeId', 'RxcCodeText', 'Total', 'SendingApplication', 'FacilityId'],
                    colModel: opts,
                    postdata: { HdrTk: 0 },
                    caption: 'DistinctRXC',
                    pager: options.pager,
            ...
            archiveGridOptions: function (options) {
                var opts = colModels().archiveColModel();
                return {
                ...
                'IntervalRepeat', 'Route', 'StartTime', 'OrcStartTime', 'EndTime', 'TransactionDate', 'OrderEffectiveDate', 'DataSource'],
                    colModel: opts,
            ...
            timesGridOpts: function (options) {
                var timeColModel = colModels().timesColModel();
                return $.extend(pharmMthds.defaultSimpleGridOptions(), {
            ...
            fieldsGridOpts: function (options) {
                var fieldsColModel = colModels().fieldsColModel();
                return $.extend(pharmMthds.defaultSimpleGridOptions(), {
            ...
            orderControlGridOpts: function (options) {
                var ocColModel = colModels().orderCtrlColModel();
                return $.extend(pharmMthds.defaultSimpleGridOptions(), {
            ...
            ordersGridOpts: function (options) {
                var ordersColModel = colModels().ordersColModel();
                return {
            ...
            siteListGridOpts: function (options) {
                ...
                return {
                    datatype: 'local',
                    colNames: ['SiteCode'],
                    colModel: [{ name: 'SiteCode', index: 'SiteCode'
                    }],
                    sortorder: 'ASC',
                    sortname: 'SiteCode',
                    caption: 'Sites',
            ...
            rxGridOpts: function (options) {
                var rxColModel = colModels().rxColModel();
                return {
            ...
            allDrugsGridOpts: function (options) {
                var alldrugsColModel = [{ name: 'SiteCode', index: 'SiteCode'}].concat(colModels().siteDrugsColModel());
                return {
                    //url: 'PharmService.asmx/SiteDrugs',
            ...
            pharmDrugsGridOpts: function (options) {
                var pharmDrugsColModel = colModels().pharmDrugsColModel();
                return {
                    ...
                    pager: options.pager,
                    afterSaveCell: function (row, cell, val, iR, iC) {
                        var caption = $('#ClassTable').jqGrid('getGridParam', 'caption');
                        if ($('#ClassTable').data(caption) === undefined) {
                            $('#ClassTable').data(caption, [])
                        }
                        $('#ClassTable').data(caption).push($('#ClassTable').jqGrid('getRowData', row));
                    }
                };
            },
            partialDrugsGridOpts: function (options) {
                var partialDrugsColModel = colModels().partialDrugsColModel();
                return {
                    //url: 'PharmService.asmx/SiteDrugs',
                    colNames: ['SiteCode', 'SiteDrugId', 'InsertedLDT', 'MappedLDT', 'SiteDrugCode', 'SiteDrugName', 'MappedPharmacyDrug', 'Strength', 'Units', 'MappedPharmacyUnit', 'isReleased', 'isReviewed'],
                    colModel: alldrugsColModel,
            ...
            profilesGridOpts: function (options) {
                var profilesColModel = colModels().profilesColModel();
                return {
                    //url: 'PharmService.asmx/SiteDrugs',
                    colNames: ['SiteCode', 'SiteDrugId', 'InsertedLDT', 'MappedLDT', 'SiteDrugCode', 'SiteDrugName', 'MappedPharmacyDrug', 'Strength', 'Units', 'MappedPharmacyUnit', 'isReleased', 'isReviewed'],
                    colModel: alldrugsColModel,
            ...
            classesGridOpts: function (options) {
                var classesColModel = colModels().classesColModel();
                return {
                    //url: 'PharmService.asmx/SiteDrugs',
                    colNames: ['SiteCode', 'SiteDrugId', 'InsertedLDT', 'MappedLDT', 'SiteDrugCode', 'SiteDrugName', 'MappedPharmacyDrug', 'Strength', 'Units', 'MappedPharmacyUnit', 'isReleased', 'isReviewed'],
                    colModel: alldrugsColModel,
